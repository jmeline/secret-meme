...
(defn series [x exp]
  (/ (reduce * (repeat exp x)) (factorial exp)))

(defn execute [x]
  (loop [arr (range 10) res 0]
    (print "arr: " arr )
    (if (empty? arr)
      (println (format " Last Number: %.4f" (float res)))
    (println (format "  res: %.4f" (float res))))
    (when-not (empty? arr)
      (recur (rest arr) (+ res (series x (first arr)))))))

((fn main [values]
  (loop [remaining values]
    (println "Remaining values: " remaining)
    (when (> remaining 0)
      (execute (Float/parseFloat (read-line)))
      (recur (dec remaining)))))4)
...
(defn execute [x]
      (loop [arr (range 10) res 0]
                (if (empty? arr)
                              (println (format "%.4f" (float res))))
                (when-not (empty? arr)
                              (recur (rest arr) (+ res (series x (first arr)))))))
((fn main[values]
       (loop [remaining (Integer/parseInt values)]
                 (when (> remaining 0)
                               (execute (Float/parseFloat (read-line)))
                               (recur (dec remaining)))))(read-line))
